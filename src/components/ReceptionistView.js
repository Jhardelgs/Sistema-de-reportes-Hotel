...
const ReceptionistView = ({ onSendReport }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    await supabase.from('reportes').insert({
      ...
      sede: newReport.location.name,
      efectivo: Number(paymentValues[1]) || 0,
      visa: Number(paymentValues[2]) || 0,
      mc: Number(paymentValues[3]) || 0,
      yape: Number(paymentValues[4]) || 0,
      transf: Number(paymentValues[5]) || 0,
      tipo_gasto: newReport.expenses[0]?.type.name || null,
      gasto_monto: newReport.expenses[0]?.amount || 0,
      descripcion: newReport.expenses[0]?.description || null,
      recepcionista: newReport.sender,
      turno: newReport.shift,
